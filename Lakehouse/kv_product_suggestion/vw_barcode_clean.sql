...
CREATE OR REPLACE VIEW kv_product_suggestion.vw_barcode_clean (
  ...
  barcode_clean)
TBLPROPERTIES (
  'transient_lastDdlTime' = '1696844762')
AS WITH
  kv_barcode AS (
SELECT
...
WHERE
    1 = 1
    AND CAST(barcode_final AS INT) IS NOT NULL ),
  barcode_char AS (
SELECT
    retailer_key,
    product_key,
    ...
    CASE
      ...
      WHEN REGEXP_LIKE(barcode_final,
        '[-]')
            AND CAST (SUBSTRING(SPLIT_PART(barcode_final, '-', 1),-1) AS int) IS NOT NULL THEN SUBSTRING(SPLIT_PART(barcode_final, '-', 1),-1)
            -- Neu chua - va phan tu dau tien sau dau - la so INT thi split va lay phan tu do
            WHEN REGEXP_LIKE(barcode_final,
            '[-]')
                AND CAST(SUBSTRING(SPLIT_PART(barcode_final,
          '-',
          2), 1, 1) AS INT) IS NOT NULL THEN SUBSTRING(SPLIT_PART(barcode_final,
        '-',
        2), 1, 1)
                -- Neu chua _ va phan tu dau tien truoc dau _ la so INT thi split va lay phan tu do
                WHEN REGEXP_LIKE(barcode_final,
                '[_]')
                    AND CAST(SUBSTRING(SPLIT_PART(barcode_final, '_', 1),-1) AS INT) IS NOT NULL THEN SUBSTRING(SPLIT_PART(barcode_final, '_', 1),-1)
        ...
        2), 1, 1)
                                        ...
                                        r'[\\]')
                                            ...
                                            r'[\\]')
                                                AND CAST(SUBSTRING(SPLIT_PART(barcode_final,
                '\\', 2), 1, 1) as INT) is not null then substring(SPLIT_PART(barcode_final, '\\', 2), 1, 1)
                                                -- Neu chua . va phan tu dau tien truoc dau . la so INT thi split va lay phan tu do
                                                WHEN REGEXP_LIKE(barcode_final,
                                                r'[.]')
                                                    AND CAST(SUBSTRING(SPLIT_PART(barcode_final, '.', 1),-1) AS INT) IS NOT NULL THEN SUBSTRING(SPLIT_PART(barcode_final, '.', 1),-1)
                ...
                AS barcode_clean,
                                                        ...
                                                        WHERE
                                                            1 = 1
                                                            AND CAST(barcode_final AS INT) IS NULL ),
combined AS (
SELECT
                *,
                CASE
                  WHEN ( (
        SELECT
            reflect('com.citigo.data.udf.udf_checkGS1',
            'checkGS1',
            barcode_char.barcode_clean) = TRUE)
            AND CAST(barcode_clean AS INT) IS NOT NULL
...
SELECT
            ...
            AND SUBSTR(barcode_clean, 1, 1) <> '8'
                ...
                SELECT
                    (CONCAT(SUBSTR(barcode_clean, 2, LENGTH(barcode_clean)-2), reflect('com.citigo.data.udf.udf_calculate_GS1_check_digit', 'calculate_GS1_check_digit', SUBSTR(barcode_clean, 2, LENGTH(barcode_clean)-2) ) )))
                ELSE
                barcode_clean
            END
                AS barcode_clean
