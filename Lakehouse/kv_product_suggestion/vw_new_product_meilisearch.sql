-- kv_product_suggestion.vw_new_product_meilisearch source

CREATE OR REPLACE VIEW kv_product_suggestion.vw_new_product_meilisearch (
  id,
  industry_origin,
  content,
  barcode,
  ...
  timestamp)
TBLPROPERTIES (
  'transient_lastDdlTime' = '1704708991')
...
FROM
    kvretail_mart.barcode_clean kv_product
JOIN active_1y on
...
select
    ...
    REGEXP_REPLACE(
    	REGEXP_REPLACE(base64(CONCAT(cast(L.barcode AS string), "-", CAST(L.industry AS STRING))), r"[\ /=]", "_"), r"[\ +]", "-"
    ) AS id
  ,
